...
`--quotation-pdf` and `--goods-receipt-pdf` are individually optional, but at least one is required. Goods-receipt export requires a valid `quotation.pendingGoodsReceiptDraft` in the input JSON. The two PDF output paths must be different. `--result-json` must not overwrite the input quotation JSON.
...
The process exits with code `0` on success and `1` on failure. It also writes one compact JSON result to standard output. A successful refresh includes the exact applied `exchangeRates` and may add `warnings` for unsupported currencies. `exchangeRateDate` is included only when the provider returns at least one rate. When `--result-json` is supplied, the same result is written to that file; this is the reliable result channel for a Windows portable GUI executable.
