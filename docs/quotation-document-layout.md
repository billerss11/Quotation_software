...
For previews with more than 100 displayed rows, the existing virtual scroller renders only the rows near the viewport and measures their variable heights. It accounts for preview zoom and the content above the table. All quotation data remains available while scrolling; this optimization is enabled only by the floating preview's context.
...
For Windows release verification, check the actual Electron preview for responsive column/template changes and preserved content, then inspect the generated PDF for page breaks, continuation context and footer containment. Include a large quotation (at least 1,000 rows) in performance checks. The PDF paginator must shallow-clone the page container and clone a row-free table shell for each page, then append only that page's rows. It must not deep-clone the full quotation for every page.
